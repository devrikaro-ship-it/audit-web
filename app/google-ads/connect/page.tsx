...
const previewRows = [
  ["Products consuming budget", "Losses ranked by cost", C.red],
  ["Profitable products without traffic", "Potential ranked by sales", C.green],
  ["Account after optimization", "Controlled-budget simulation", C.cyan],
] as const;
    ...
    <div {...publicOAuthAttributes("connect", message ? "error" : "normal")} className="min-h-dvh px-5 py-8 sm:px-8 sm:py-10" style={{ fontFamily: inter, background: "linear-gradient(145deg,#f7f7fd 0%,#ffffff 58%,#f1fbfc 100%)" }}>
      <div className="mx-auto max-w-[1120px]">
        ...
        <main className="mt-8 grid overflow-hidden rounded-[28px] border bg-white lg:grid-cols-[0.92fr_1.08fr]" style={{ borderColor: C.border, boxShadow: "0 28px 80px rgba(19,22,58,0.10)" }}>
          <section className="relative overflow-hidden px-7 py-10 sm:px-10 sm:py-12 lg:px-12 lg:py-14" style={{ background: C.navy }}>
            ...
            <div className="relative">
              <p className="text-xs font-extrabold uppercase tracking-[0.14em]" style={{ color: C.cyan }}>The final step before analysis</p>
              <h1 className="mt-5 max-w-[480px] text-3xl font-black leading-[1.08] tracking-[-1.2px] text-white sm:text-4xl" style={{ fontFamily: sora }}>
                {notConfigured ? "The connection will be available soon" : "Connect your account and see how every product performs"}
              </h1>
              <p className="mt-5 max-w-[500px] text-base leading-relaxed" style={{ color: "rgba(255,255,255,0.72)" }}>The audit reads the last 365 days and shows, by name and numbers, which products lose money, which deserve more traffic, and how the budget could be redistributed.</p>
              ...
              <div className="mt-9 overflow-hidden rounded-2xl border" style={{ borderColor: "rgba(255,255,255,0.14)", background: "rgba(255,255,255,0.07)" }}>
                <div className="border-b px-5 py-4" style={{ borderColor: "rgba(255,255,255,0.12)" }}><p className="text-xs font-bold uppercase tracking-[0.12em]" style={{ color: "rgba(255,255,255,0.55)" }}>What you will see in the report</p></div>
          ...
          <section className="px-7 py-10 sm:px-10 sm:py-12 lg:px-12 lg:py-14">
            ...
            {notConfigured ? (
              <>
                <p className="text-base leading-relaxed" style={{ color: C.gray500 }}>The automated audit is in its final preparation stage. Until then, we can analyze the same figures manually.</p>
                <a href="mailto:hello@devrika.ro?subject=Google%20Ads%20Shopping%20Audit" className="mt-7 inline-flex min-h-14 items-center rounded-xl px-7 text-base font-extrabold text-white" style={{ background: brandGradient }}>Contact us for an audit</a>
              </>
            ) : (
              <>
                ...
                <form action="/api/google-ads/start" method="get" className="mt-7">
                  ...
                  <input id="website" name="website" type="url" required placeholder="https://your-store.com" className="mt-2 min-h-14 w-full rounded-xl border px-4 text-base outline-none focus:ring-2" style={{ borderColor: C.border, color: C.navy }} />
                  <p className="mt-2 text-left text-xs leading-relaxed" style={{ color: C.gray500 }}>We use this address only to identify the store in your report. The actual average order value comes from Google Ads Purchase conversions.</p>
                  <button type="submit" className="mt-6 inline-flex min-h-14 w-full items-center justify-center gap-2.5 rounded-xl px-7 text-base font-extrabold text-white transition-transform hover:-translate-y-0.5 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 motion-reduce:transition-none motion-reduce:hover:translate-y-0" style={{ background: brandGradient, boxShadow: "0 12px 30px rgba(71,73,158,0.24)", outlineColor: C.indigo }}>Continue securely with Google<svg aria-hidden="true" width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M5 12h14M12 5l7 7-7 7" /></svg></button>
                </form>

                <div className="mt-7 grid gap-3 sm:grid-cols-3">
                  {["We do not change campaigns or budgets", "We do not request Gmail or Drive", "Revoke access at any time"].map((assurance) => (
                ...
                <details data-oauth-disclosure="progressive" className="group mt-7 border-t pt-5" style={{ borderColor: C.border }}>
                  <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-bold" style={{ color: C.indigo }}>Exactly what data we read and what Google access permits<span aria-hidden="true" className="text-lg transition-transform group-open:rotate-45">+</span></summary>
                  <div className="mt-4 space-y-3 text-left text-xs leading-relaxed" style={{ color: C.gray500 }}>
                    ...
                    <p>We do not request access to Gmail, Drive, or other services. You can revoke access at any time from your Google account.</p>
                  </div>
                </details>
              </>
            )}
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-4 text-xs" style={{ color: C.gray500 }}>
          <Link href="/google-ads" className="font-bold hover:underline" style={{ color: C.indigo }}>← Back to the audit page</Link>
          <span>No card · No subscription · Saved PDF report</span>
        </div>
      </div>
    </div>
  );
