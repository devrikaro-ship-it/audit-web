...
vi.mock("@/lib/gads-leads", () => ({ saveLead, updateLead }));
...
it("keeps the saved PDF and reports email failure honestly", async () => {
  ...
  expect(updateLead).toHaveBeenCalledWith("lead-1", expect.objectContaining({ deliveryStatus: "PDF_READY", pdfPath: "/data/report.pdf" }));
