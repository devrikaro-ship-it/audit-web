...
export const reportContract = {
  ...
  surfaces: {
    "navigation": { rendering: "always-on-success", dependencies: [] },
    "demo-banner": { rendering: "conditional", dependencies: ["isDemo"], projection: ["demo"], guard: "demoBanner", predicate: "isDemo" },
    "headline-summary": { rendering: "always-on-success", dependencies: [] },
    "money-findings": { rendering: "always-on-success", dependencies: [] },
    "catalog-map": { rendering: "conditional", dependencies: ["catalogCount"], projection: ["hartiCatalog.length"], guard: "catalogMap", predicate: "catalogCount>=1" },
    "account-settings": { rendering: "conditional", dependencies: ["repairPointCount"], projection: ["rep.puncte.length"], guard: "accountSettings", predicate: "repairPointCount>=1" },
    "unsupported-conclusions": { rendering: "conditional", dependencies: ["quarantinedCount"], projection: ["nejudecabile.length"], guard: "unsupportedConclusions", predicate: "quarantinedCount>=1" },
    "simulator-call-to-action": { rendering: "conditional", dependencies: ["hasStructure", "currentRoas"], projection: ["Boolean(structura)", "structura?.roasCont??0"], guard: "simulator", predicate: "hasStructure&&currentRoas>0" },
