...
const C = {
  ...
  red: "#C0392B", orange: "#D45B00", green: "#1A7A4A",
  g400: "#8FA3C0", g500: "#64748b", g600: "#4A5E7A", g800: "#1E2D42",
  slate: "#F4F6FB", bord: "#E6EBF4", white: "#FFFFFF",
};
...
function ChannelCard({ c }: { c: WarmChannel }) {
  ...
  return (
    <div style={{ background: C.white, border: `1px solid ${C.bord}`, borderRadius: 18, padding: 22, boxShadow: "0 8px 30px rgba(19,22,58,.08)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        {c.kpis.map((k, i) => (
          <div key={i} style={{ background: C.slate, borderRadius: 11, padding: 12 }}>
            <div style={{ fontFamily: sora, fontSize: 21, fontWeight: 800, color: k.color ?? C.g800 }}>{k.v}</div>
            <div style={{ fontSize: 11.5, color: C.g500, marginTop: 1 }}>{k.l}</div>
          </div>
        ))}
      </div>
      {s != null && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11.5, color: C.g500, marginBottom: 5 }}>
      ...
      {c.note && <p style={{ fontSize: 13.5, color: C.g600, lineHeight: 1.5, marginTop: 13 }}>{c.note}</p>}
    </div>
  );
}
...
function FindingCard({ f, idx }: { f: WarmFinding; idx: number }) {
  ...
  return (
    <div style={{ background: C.white, border: `1px solid ${C.bord}`, borderRadius: 16, padding: "22px 26px", boxShadow: "0 6px 24px rgba(19,22,58,.05)", display: "flex", gap: 18, marginBottom: 14 }}>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8, flexWrap: "wrap" }}>
          ...
          {f.meta && <span style={{ fontSize: 12, color: C.g500, textTransform: "uppercase", letterSpacing: ".04em" }}>{f.meta}</span>}
        </div>
        <h4 style={{ fontSize: 17, fontWeight: 700, color: C.g800, margin: "0 0 7px" }}>{f.title}</h4>
        <p style={{ fontSize: 14.5, color: C.g600, lineHeight: 1.55, margin: "0 0 10px" }}>{f.problema}</p>
        {f.fix && <p style={{ fontSize: 13.5, color: C.g800, lineHeight: 1.5, margin: 0 }}><b style={{ color: C.indigo }}>Ce facem:</b> {f.fix}</p>}
      </div>
    </div>
  );
}
...
function Section({ title, sub, items }: { title: string; sub?: string; items?: WarmFinding[] }) {
  ...
  return (
    <section style={{ padding: "48px 0 0" }}>
      ...
      {sub && <p style={{ color: C.g500, fontSize: 15, margin: "6px 0 22px" }}>{sub}</p>}
...
export function WarmReportRenderer({ d }: { d: WarmReport }) {
  ...
  return (
    <div style={{ fontFamily: inter, background: C.slate, minHeight: "100vh", color: C.g800 }}>
      <header style={{ background: `radial-gradient(120% 120% at 50% -10%, ${C.navyMid} 0%, ${C.navy} 60%)`, color: C.white, padding: "30px 0 56px" }}>
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ fontSize: 13, color: C.g400 }}>{d.date}</span>
          </div>
            ...
            <div style={{ maxWidth: 780, margin: "26px auto 0", background: strike ? "rgba(192,57,43,.12)" : "rgba(10,190,207,.08)", border: strike ? "1px solid rgba(255,140,140,.3)" : "1px solid rgba(10,190,207,.22)", borderRadius: 18, padding: "22px 24px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: 200, textAlign: "center" }}>
                  <div style={{ fontSize: 11.5, letterSpacing: ".06em", textTransform: "uppercase", color: C.g400, marginBottom: 4 }}>{gun.left_lab ?? "Platforma raporteaza"}</div>
                ...
                <div style={{ fontFamily: sora, fontWeight: 700, fontSize: 12, color: C.g400, padding: "0 6px", textTransform: "uppercase" }}>{gun.vs ?? "vs realitate"}</div>
                <div style={{ flex: 1, minWidth: 200, textAlign: "center" }}>
                  <div style={{ fontSize: 11.5, letterSpacing: ".06em", textTransform: "uppercase", color: C.g400, marginBottom: 4 }}>{gun.right_lab ?? "GA4 — vanzari reale"}</div>
          ...
          <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap", marginTop: 24 }}>
            {[{ v: t.cpa ?? "—", l: "CPA target" }, { v: t.troas ?? "—", l: "tROAS target" }, { v: t.aov ?? "—", l: "AOV (GA4)" }, ...(t.business ? [{ v: t.business, l: t.business_l ?? "venit real / 90z" }] : [])].map((x, i) => (
              <div key={i} style={{ background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.1)", borderRadius: 14, padding: "14px 24px", textAlign: "center" }}>
                ...
                <div style={{ fontSize: 12, color: C.g400, marginTop: 2 }}>{x.l}</div>
              </div>
            ))}
          </div>
          <p style={{ maxWidth: 720, margin: "20px auto 0", textAlign: "center", fontSize: 12.5, color: C.g400, lineHeight: 1.5 }}>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "0 24px" }}>
        ...
        {d.opportunity && (
          <div style={{ background: "linear-gradient(135deg,rgba(10,190,207,.08),rgba(71,73,158,.08))", border: `1px solid ${C.bord}`, borderLeft: `4px solid ${C.cyan}`, borderRadius: 16, padding: "24px 26px", marginTop: 18 }}>
            ...
            {d.opportunity.b && <Html html={d.opportunity.b} style={{ display: "block", fontSize: 14, color: C.g600, lineHeight: 1.55 }} />}
          </div>
        )}

        {d.proof && d.proof.length > 0 && (
          <div style={{ marginTop: 18, background: C.white, border: `1px solid ${C.bord}`, borderRadius: 16, padding: "20px 24px" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 9 }}>
              {d.proof.map((p, i) => <Html key={i} html={p} style={{ fontSize: 12.5, color: C.g800, background: C.slate, border: `1px solid ${C.bord}`, borderRadius: 9, padding: "7px 12px" }} />)}
            </div>
          </div>
        )}
        ...
        {d.quickwins && d.quickwins.length > 0 && (
          <section style={{ padding: "48px 0 0" }}>
            ...
            <p style={{ color: C.g500, fontSize: 15, margin: "6px 0 22px" }}>Efect imediat, fara dependente. De-aici incepe.</p>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 12 }}>
              {d.quickwins.map((w, i) => (
                <div key={i} style={{ background: C.white, border: `1px solid ${C.bord}`, borderTop: `3px solid ${C.cyan}`, borderRadius: 14, padding: 18 }}>
                  <div style={{ fontFamily: sora, fontWeight: 800, color: C.cyan, fontSize: 13, marginBottom: 6 }}>PAS {i + 1}</div>
                  <div style={{ fontFamily: sora, fontWeight: 700, fontSize: 14.5, color: C.g800, marginBottom: 5 }}>{w.t}</div>
                  {w.d && <div style={{ fontSize: 13, color: C.g600, lineHeight: 1.5 }}>{w.d}</div>}
                </div>
              ))}
            </div>
          </section>
        ...
        {d.plan && d.plan.length > 0 && (
          <section style={{ padding: "48px 0 0" }}>
            ...
            <p style={{ color: C.g500, fontSize: 15, margin: "6px 0 22px" }}>In ordine — tracking-ul intai, apoi structura.</p>
            <div style={{ background: C.white, border: `1px solid ${C.bord}`, borderRadius: 18, padding: 26 }}>
              {d.plan.map((p: WarmStep, i) => (
                <div key={i} style={{ display: "flex", gap: 14, padding: "12px 0", borderBottom: i < d.plan!.length - 1 ? `1px solid ${C.bord}` : "none" }}>
                  ...
                  <div>
                    <b style={{ fontFamily: sora, fontSize: 15, color: C.g800, display: "block" }}>{p.t}</b>
                    {p.d && <span style={{ fontSize: 13.5, color: C.g600 }}>{p.d}</span>}
                  </div>
                </div>
              ))}
            </div>
      ...
      <footer style={{ textAlign: "center", color: C.g400, fontSize: 13, padding: "36px 0 48px" }}>
