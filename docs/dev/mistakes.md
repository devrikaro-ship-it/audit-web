...
## 2026-09-24 — Production PDFs came out in a fallback font: Chrome printed on a time budget, not when the page was ready

Symptom: in the afternoon every production PDF (16:9 and phone) used DejaVu fallbacks; a production PDF of the same report that morning had Barlow, and every local PDF had Barlow. Measured cause: the route ran `chrome --print-to-pdf --virtual-time-budget=15000`, which prints when the budget runs out, not when the report (rendered after a client fetch) and its fonts are ready. Reproduced locally with chrome-headless-shell 132 (the production version): fallback fonts; the same Chrome printing after `document.fonts.ready`: Barlow. A first repair (fonts inlined into a print stylesheet) was built on the wrong hypothesis that the font download failed, did not change production, and was reverted. Recognition signal: a PDF whose embedded fonts are fallbacks while the page on screen shows Barlow. Repair: `printReportPdf` drives Chrome through playwright-core, waits for the slides and `document.fonts.ready`, then prints; its test serves a page whose slides appear late and whose font answers slowly, and fails without either wait. Reproduce with the production browser version before choosing a repair; check embedded fonts on production after a deploy that touches the report.
