...
**Operational ramas:** cheie PageSpeed in env Coolify, restrictie zona BrightData la IP Hetzner la deploy. **De reglat dupa date reale:** constantele din `roi-sim.ts` (CPC_BENCH, tinte conversie) — usor de schimbat, sus in fisier.
...
**Neatacat (constient):** garda SSRF (scan/audit fac fetch pe URL-uri arbitrare server-side — chokepoint pregatit in `lib/net.ts`, dar validarea de IP nu e pusa); componenta comuna `FindingCard` intre RECE↔CALD (design tokens impartasite, dar structura cardului inca dublata). PSI ruleaza fara API key → 429 pe volum (limita externa, nu bug).
