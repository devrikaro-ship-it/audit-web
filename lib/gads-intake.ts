...
/** Fereastra de analiza. 12 luni = un an intreg de sezonalitate, cum cere SPEC-ul. */
...
/** Interval [acum-365z, azi] in formatul cerut de GAQL. `today` injectabil pentru teste. */
export function dateRange(today = new Date()): { from: string; to: string } {
...
export function perfQuery(from: string, to: string): string {
  return `SELECT shopping_product.item_id, shopping_product.title,
          metrics.impressions, metrics.cost_micros, metrics.conversions_value
          FROM shopping_product
...
type RawRow = {
  ...
  metrics?: { impressions?: string | number; costMicros?: string | number; conversionsValue?: string | number };
};
