...
describe("paginateChecklist", () => {
  ...
  it("with the ten SEO components: numbered components, faults in chain order, unmeasured rows de verificat", () => {
    ...
    expect(d.seo.problems.map((p) => [p.title, p.count])).toEqual([["robots.txt nu spune unde e lista de pagini", "de reparat"], ["La 40 din 40 de pagini de produs, pozele produsului nu au un text care sa spuna ce arata", "de reparat"]]);
    expect(d.seo.standard?.map((g) => [g.name, g.score, g.rows.map((r) => [r.state, r.title, r.result])])).toEqual([
      ...
      ["2. Ce ii permite site-ul lui Google (robots.txt)", 0, [["fail", "robots.txt nu spune unde e lista de pagini", "de reparat"]]],
      ["3. Informatia e scrisa direct in pagina", null, [["verify", "Pretul e scris direct in pagina", "de verificat"]]],
      ["4. Textul paginii", 0, [["fail", "La 40 din 40 de pagini de produs, pozele produsului nu au un text care sa spuna ce arata", "de reparat"]]],
    ]);
    ...
    expect(d.first?.title).toBe("robots.txt nu spune unde e lista de pagini");
